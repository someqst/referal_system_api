...
async def register_user(user_data: UserSchema, response: Request, session = Depends(get_db)):
    try:
...
async def auth_user(response: Response, user_data: UserSchema, session = Depends(get_db)):
    try:
        ...
        check = await authenticate_user(password=user_data.password, hashed_password=user.password)
        if not check:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
